...
\title{\Huge{MATH1081}\\Advanced Discrete Mathematics}
\author{\huge{Michael Kasumagic, s4430266}}
\date{\huge{Semester 1, 2025}}
...
\begin{itemize}
  \item ``Not continuous'' (e.g., not calculus)
  \item Logic, prime numbers, abstract networks, map colouring, knots
  \item Computer science, philosophy, cryptography
  \item Things you could describe exactly to a computer as a sequence of bits
  \item Answers are not enough... we require \textit{proofs}
\end{itemize}

\subsubsection{Logic: An ``algebra for knowledge and deduction''}
\begin{itemize}
  \item Allows us to express overselves precisely 
  $$
    ((p\land\lnot q)\lor(p\land q))\land q \equiv p \land q
  $$
  \item This is crucual for definitions, theorems, and proofs
  \item Underpins much of computer science (which is a logical deduciton machine)
  \item \textbf{G\"odel's Incompleteness theorem} (1931). There are true statements which we can't prove.
\end{itemize}
